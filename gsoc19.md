...
This command would be helpful when someone wants to go a particular part of the conversation and roughly remember when the conversation took place (days-ago, date, time of the conversation). The user can then use /sb command with 'goto' argument along with <+-linecount> or <linenum> or <timestamp>, to go to that part of the conversation in the tab.
